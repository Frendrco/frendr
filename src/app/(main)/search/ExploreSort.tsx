...
export function ExploreSort({ current }: { current: SortValue }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 z-50 min-w-[130px] overflow-hidden rounded-xl border border-border bg-background shadow-md">
          {OPTIONS.map(({ value, label }) => (
            ...
            >
              <span className={current === value ? "text-foreground font-medium" : "text-foreground/60"}>
                {label}
              </span>
              {current === value && <Check size={13} className="text-foreground" />}
            </button>
          ))}
        </div>
      )}
