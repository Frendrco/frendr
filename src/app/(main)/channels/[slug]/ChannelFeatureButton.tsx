...
export function ChannelFeatureButton({ channelId, initialFeatured }: { channelId: string; initialFeatured: boolean }) {
  ...
  async function toggle() {
    if (loading) return
    setLoading(true)
    const res = await fetch(`/api/channels/${channelId}/feature`, { method: "POST" })
    if (res.ok) {
  ...
  return (
    <button
      onClick={toggle}
      disabled={loading}
      aria-label={featured ? "Remove from Featured Channels" : "Feature this channel"}
      title={featured ? "Remove from Featured Channels" : "Feature this channel"}
      className={cn(
