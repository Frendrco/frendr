...
export default async function ChannelFollowersPage({ params }: Props) {
    ...
    ? new Set(
        (await prisma.follow.findMany({
          where: {
            ...
            followingId: { in: channel.followers.map((f) => f.user.id) },
          },
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-screen-xl px-4 md:px-6 py-10">
        ...
        >
          <ArrowLeft size={13} /> {channel.name}
        </Link>
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 font-sans text-sm text-foreground/40">
            {followers.length === 0
              ...
              : `${followers.length} ${followers.length === 1 ? "person follows" : "people follow"} ${channel.name}`}
          </p>
        </div>
